# Connector Expansion Design — v2
**Date:** 2026-04-16
**Project:** CivicRecords AI
**Status:** Pending final approval — ready for implementation planning
**Spec ref:** UNIFIED-SPEC.md §11.4–11.5
**Supersedes:** `2026-04-16-connector-expansion-design.md` (v1)

**Revision history:**
- v1 — initial design doc (4 audit gates passed, Gate 4 failed, 3 gaps)
- v2 — addresses all 7 audit findings: Alembic downgrade, 429 retry policy, response_format field, recursive JSONB walk, §2.5/§2.6 renumbering, 5xx rationale, discover() mime_type hint
...
- `connectors/retry.py` — shared HTTP retry utility
...
5. Emits one `DiscoveredRecord` per row:
   ...
   - `mime_type` = `"application/json"` (ODBC rows are always serialized to JSON)
6. Updates cursor **only on full successful completion** (see §2.6 cursor semantics).
...
This policy must be implemented as a shared utility (`connectors/retry.py`) used by both `RestApiConnector` and any future HTTP-based connectors. It must not be re-implemented per connector.
...
class RestApiConfig(BaseModel):
    ...
    # Governs mime_type on DiscoveredRecord (hint) and FetchedDocument (authoritative).
    ...
    # Binary / multipart responses are out of scope for v1 (see §1 out-of-scope).
...
class ODBCConfig(BaseModel):
    ...
    connection_string: str      # CREDENTIAL — masked in UI after save, never returned in API responses
                                ...
                                # Frontend code MUST flag this field with a comment — it is not
...
- `test_discover_429_retry` — connector retries on rate limit; respects `Retry-After` header
...
- `test_partial_sync_cursor_not_advanced` — fetch fails on record N; asserts last_sync_cursor unchanged (lives in `test_ingestion_tasks.py` — see §5.3)
- `test_connection_safety` — calls test-connection path; asserts: (1) `data_sources.connection_config` is byte-for-byte unchanged in the DB, (2) no credential field values (`api_key`, `client_secret`, `password`, `token`) appear anywhere in any `AuditLog.details` JSONB entry written during the call. Query: `session.query(AuditLog).filter(AuditLog.action == "test_connection").all()`. The credential containment check **must recursively walk all nested string values** in each `.details` dict — including values inside arrays and sub-objects at any depth (e.g., `details["request_context"]["headers"]["Authorization"]`) — and fail if any credential string appears as a substring of any string value at any depth. A flat `entry.details.get("api_key")` check is explicitly prohibited: it passes against an implementation that logs the credential one level deeper.
...
- `test_health_check`
- `test_partial_sync_cursor_not_advanced` — fetch fails mid-run; cursor remains at pre-run value (lives in `test_ingestion_tasks.py` — see §5.3)
- `test_connection_safety` — same as REST variant: `connection_config` unchanged in DB; `connection_string` value does not appear as a substring of any string value **at any depth** in any `AuditLog.details` JSONB entry written during the call. Uses the same recursive-walk helper as the REST variant — do not duplicate the walk logic.

### 5.3 Cursor semantics test — `test_ingestion_tasks.py`

The `test_partial_sync_cursor_not_advanced` test belongs in `test_ingestion_tasks.py`, not the connector unit tests, because cursor writes happen at the sync runner level (`ingestion/tasks.py`), not inside the connector.
...
- [ ] `backend/app/connectors/retry.py` — shared HTTP retry utility (§2.4 policy)
...
- [ ] `frontend/` — wizard Step 2 branching for new connector types; mask `connection_string` with inline comment

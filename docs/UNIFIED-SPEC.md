...
5. Connector expansion — **DONE (d335c5b).** `RestApiConnector` + `OdbcConnector` shipped. 61 connector tests passing. See `CHANGELOG.md`.
   5a. **P6a — Idempotency contract split** — IN DESIGN. See `docs/superpowers/specs/2026-04-16-p6a-idempotency-design.md`. Correctness fix: REST/ODBC connectors produce non-deterministic file hashes; dedup contract must split by connector type. Ships before P6b/P7.
   5b. **P6b — Cron scheduler rewrite** — IN DESIGN. See `docs/superpowers/specs/2026-04-16-p6b-scheduler-design.md`. Replaces drift-prone `schedule_minutes` interval with `sync_schedule` cron expressions via croniter.
   5c. **P7 — Sync failures, circuit breaker, UI polish** — IN DESIGN. See `docs/superpowers/specs/2026-04-16-p7-sync-failures-design.md`. Per-record failure tracking, circuit breaker, Option B card layout, failed records panel.
...
Decisions that constrain implementation. Each links to the test that proves it. Future devs: if you want to change a decision, update the test first.

| ID | Decision | Why | Proof Test |
|---|---|---|---|
| D-IDEM-1 | Split idempotency contract: binary connectors dedup by `(source_id, file_hash)`; structured connectors (REST/ODBC) dedup by `(source_id, source_path)`. | REST/ODBC hash non-determinism confirmed — raw response bytes include rotating envelope fields. Same CDC/ETL pattern as Airbyte/Fivetran. | `test_pipeline_idempotency.py` — determinism tests |
| D-IDEM-2 | `data_key` is required config on RestApiConfig, not optional. Dotted-path syntax only (no JSONPath). Default null = hash root object. | `sort_keys=True` fixes key ordering but not envelope pollution. One misconfigured API re-breaks the contract. | `test_rest_connector.py` — data_key tests |
| D-IDEM-3 | Test-connection calls fetch() twice (500ms apart), warns on hash mismatch with differing key list. | Converts silent production duplicate-write bug to test-time guardrail. Admin finds out during config, not 3 weeks post-GA. | `test_datasources_router.py` — pollution detection tests |
| D-IDEM-4 | source_path construction rules frozen at GA. ODBC: `{table}/{url_encoded_pk}`. REST: `{base_url}{endpoint_path}/{url_encoded_id}`. Max 2048 chars. | Path format changes after GA = silent duplicate documents across environments. Frozen rules prevent this. | `test_rest_connector.py`, `test_odbc_connector.py` — source_path encoding tests |
| D-IDEM-5 | Upstream source_path change (API re-key, PK change) = new document, orphaned old row. No fuzzy matching. | Fuzzy matching introduces its own failure modes. Document the behavior; don't hide it. | `test_pipeline_idempotency.py` — identity change test |
| D-IDEM-6 | Deletion detection is a Known Gap in v1. Missing records not flagged. | Out of scope; flag for v1.2. | N/A — Known Gap |
| D-SCHED-1 | `sync_schedule` (cron, via croniter) replaces `schedule_minutes`. Separate `schedule_enabled` boolean preserves expression when toggled off. | Interval scheduling drifts. Municipal admins think in clock time. "Run at 2am" is a cron expression, not an interval. | `test_scheduler.py` |
| D-SCHED-2 | Min interval validated via rolling 7-day window sampling, not just next-interval check. Floor: 5 minutes. | Adversarial cron `*/1 0 * * *` passes a single get_next() check. Week sampling catches it. | `test_scheduler.py` — min interval adversarial test |
| D-FAIL-1 | Two retry layers: task-level (3 retries, 30s→90s→270s, 10-min cap) for transient errors; record-level (5 retries, one per tick, N=100/T=90s per-run cap) for persistent failures. Handoff: task exhaustion → sync_failures row inserted. | Task-level prevents VPN hiccups from polluting sync_failures. Without task-level, circuit breaker trips from noise. | `test_sync_runner_retry_layers.py` |
| D-FAIL-2 | Partial failure cursor semantics: cursor advances past successfully-processed records. Failed records tracked in sync_failures by source_path. | All-or-nothing cursor = one poisoned record freezes source forever, causes re-fetch of full dataset nightly. | `test_sync_runner_cursor.py` |
| D-FAIL-3 | Retry ordering per tick: existing sync_failures ('retrying') processed before discover() for new records. Per-run cap: N=100 records OR T=90s. | Resolving existing failures > expanding backlog. Cap prevents worker pile-up on large failure queues. | `test_sync_runner_retry_cap.py` |
| D-FAIL-4 | Circuit breaker rule: full-run failure = authenticate() throws, OR discover() throws, OR every fetch (new + retry) fails. Zero-work runs (discover=0, no pending retries) do not move counter. Any single success resets counter to 0. | Ambiguous semantics produce counter drift. Explicit rule prevents false positives (0 records = success) and false negatives (partial success = full failure). | `test_circuit_breaker.py` — edge case tests |
| D-FAIL-5 | Unpause grace period: threshold=2 for first post-unpause sync window, then returns to 5. | Admin who unpauses with creds still wrong sees immediate feedback (2 failures), not 5. "Unpause didn't work" confusion prevented. | `test_circuit_breaker.py` — grace period test |
| D-FAIL-6 | Dismiss = soft delete. status='dismissed' with dismissed_at, dismissed_by. Hard deletes prohibited. | "We chose not to ingest this record" is a compliance artifact for a civic records product. | `test_sync_failures.py` — dismiss test |
| D-FAIL-7 | 404 during task-level retry = tombstone (not retrying). Persistent 404 = record deleted upstream. Not re-queued. | Chasing deleted records wastes worker capacity and confuses admins. Tombstone is the correct semantic. | `test_sync_failures.py` — 404 tombstone test |
| D-FAIL-8 | health_status computed at API response time from stored fields (consecutive_failure_count, sync_paused) + LEFT JOIN count on sync_failures. Not a stored field. | Avoids cache staleness. Two stored inputs are always current; join adds one query, not N. | `test_datasources_router.py` — health_status tests |
| D-FAIL-9 | sync_run_log table: one row per run, no coupling to retry logic. Minimal fields. | "Why did this sync run at 2:13 not 2:00" is unanswerable without a run log. Conflating run log with retry tracking (via sync_failures) would complicate both. | `test_sync_run_log.py` |
| D-UI-1 | Sync Now button stays in "Syncing…" state until actual completion detected (5s polling on last_sync_at). Timeout at 5min shows "taking longer than expected." | Button that lies about completion state is a shipped bug. Admin hammers a button that says "done" while sync still runs. | Manual QA — no automated test possible |
| D-UI-2 | Notification recipients: created_by user. Fallback to all ADMIN-role users if created_by is inactive. Triggers: circuit-open (required), recovery after unpause+success (required). First-failure: not triggered. | First-failure notification is noisy (every VPN blip). Circuit-open is the signal that matters: admin must act. | `test_sync_notifications.py` |

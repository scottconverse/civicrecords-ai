...
What remains: at-rest encryption for `connection_config` (ENG-001 / Tier 6); first-boot truth cleanup and full-spectrum installer (Tier 5, including T5E); public-facing portal; full active network discovery; GIS connector. Connector taxonomy unification (T3B) and test-connection truthfulness (T3C) are complete as of CI run 24727777171 (569 tests, 0 failures). OpenAPI typegen (T3D) and UI runtime + responsive AppShell + accessible Add Source wizard + Dashboard/Settings service-health fix (Tier 4 — T4A, T4B, T4C) are implemented locally post that CI run and verified via vitest (19/19 frontend tests pass, zero React ref warnings) plus browser QA at 480×900 and 1280×900.

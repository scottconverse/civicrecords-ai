...
### Added
- **P7 QA pass — Retry-After robustness, grace period activation, ARIA accessibility (`301c4f3`, 2026-04-17):** Four bug fixes plus accessibility improvements shipped together. (1) **Retry-After crash fix:** `with_retry()` now catches `ValueError`/`TypeError` on malformed `Retry-After` headers (e.g. `"banana"`, empty string) and falls back to exponential backoff instead of raising; the 600s cap (D-FAIL-12) is enforced even on valid numeric values. (2) **Grace period activation fix:** `_grace_period_active` was a transient Python attribute checked but never set — Celery workers don't share in-memory state with the API process. Fixed by using the existing DB-persisted `sync_paused_reason` column as the sentinel: unpause sets `sync_paused_reason="grace_period"`; sync runner reads it to apply threshold=2; clears it to `None` on first successful sync. (3) **ARIA accessibility on `SourceCard` and `FailedRecordsPanel`:** health dot gets `role="img"` + `aria-label="Health: <state>"` so screen readers announce it without relying on color alone; visible label gets `aria-hidden="true"`; Sync Now button gets dynamic `aria-label` during sync with elapsed time; sync timer wrapped in `aria-live="polite" aria-atomic="true"`; failed records wrapper gets `role="region"` + `aria-label`; error state in panel gets `role="alert"`. (4) **Copy fix:** `⚠ Paused` → `⚠️ Paused` in `ScheduleLabel`. 4 adversarial Retry-After tests added in `test_rest_connector.py`; 2 grace-period integration tests added in `test_circuit_breaker.py` (require Postgres, run in CI). 39 unit tests passing.
- **Rule 9 mandatory deliverables (`c433beb`, 2026-04-17):** UML architecture diagrams (6 Mermaid `.mmd` source files + 6 SVG renders in `docs/diagrams/`): Class, Component, Sequence, Deployment, Activity — all produced at professional grade with `render_diagrams.py`. README produced in all four formats: `.md`, `.txt`, `.docx`, `.pdf` (docx/pdf embed the UML diagrams inline). `USER-MANUAL.md` scaffolded. Landing page (`docs/index.html`) redesigned with 5 required action buttons (GitHub Repo, Download Installer Linux, Download Installer Windows, User Manual, README). GitHub Discussions seed file at `docs/github-discussions-seed.md` (Announcements, Q&A, Ideas, Show and Tell, General — 9 posts).
- **USER-MANUAL.md complete (`9c1d98b`, 2026-04-17):** Three-section user manual: Section A (End-User Guide for city clerks — plain language, step-by-step walkthroughs of every user-facing feature, error reference, glossary), Section B (Technical Reference for IT admins — full env-var reference, Docker service inventory, backup/restore, connector config, troubleshooting), Section C (Architectural Reference — all 6 UML diagrams with written explanation of what each shows, data flow, design rationale). Produced in `.md`, `.docx`, and `.pdf`.

### Fixed
- **Landing page User Manual link (`23f0655`, 2026-04-17):** Both User Manual action buttons in `docs/index.html` pointed to `civicrecords-ai-manual.pdf` (a file that does not exist). Corrected to `../USER-MANUAL.pdf`.
